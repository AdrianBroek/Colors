...
function randomColor(){
    ...
    colorDivs.forEach((div, index) => {
        ...
        const generator = generateHex();
    //add it to the array
    if (div.classList.contains('locked')){
        initialColors.push(hexText.innerText);
        return;
    }   else   {
        initialColors.push(chroma(generator).hex());
    }

    //adding colors
        hexText.innerText = generator;
        div.style.background = generator;
        checkTextContrast(generator, hexText);
    //initial colorize sliders
        const color = chroma(generator);
        const selectors = div.querySelectorAll('input');
        const hue = selectors[0];
        const bright = selectors[1];
        const sat = selectors[2];
...
function colorizeSliders(color, hue, bright, sat){
    ...
    //Update Input Colors
    sat.style.backgroundImage = `linear-gradient(to right, ${scaleSat(0)}, 
    ${scaleSat(1)} )`;
    bright.style.backgroundImage = `linear-gradient(to right, ${scaleBright(0)},
    ${scaleBright(0.5)}, ${scaleBright(1)})`;
    hue.style.backgroundImage = `linear-gradient(to right, rgb(204,75,75), 
    rgb(204,204,75), rgb(75,204,75), rgb(75,204,204), rgb(75,75,204), rgb(204,75,204), rgb(204,75,75))`;
}
